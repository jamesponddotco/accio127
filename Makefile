...
doc: # Builds the manpage.
	$(SCDOC) <cmd/accio127ctl/doc/accio127ctl.1.scd >accio127ctl.1

install: # Installs the release binary.
	$(INSTALL) -d \
		$(DESTDIR)$(PREFIX)/$(BINDIR)/ \
		$(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
	$(INSTALL) -pm 0755 accio127ctl $(DESTDIR)$(PREFIX)/$(BINDIR)/
	$(INSTALL) -pm 0644 accio127ctl.1 $(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
...
clean: # Cleans cache files from tests and deletes any build output.
	$(RM) -f cover.out accio127ctl accio127ctl.1
